...
def get_tonic_at_time(onset: float, harmony_map: list[tuple[float, int, bool]], harmony_times: list) -> Tuple[int, bool]:
    if not harmony_map:
        return (0, False)
...
def generate_fuzzy_pairs(bass_notes: list[NoteInfo], soprano_notes: list[NoteInfo], harmony_file: str | Path, max_sync_distance: float = 2.0, grid_size: float | None = 0.5, min_duration: float = 0.5) -> Iterator[ContrapuntalPair]:
    """
    ...
    for bass in bass_notes:
        ...
        # Move index forward
        while soprano_idx < num_sopranos and soprano_notes[soprano_idx].onset < (bass.onset - max_sync_distance):
            soprano_idx += 1
        ...
        for i in range(soprano_idx, num_sopranos):
            ...
            current_tonic, is_minor = get_tonic_at_time(bass.onset, harmony_map, harmony_times)
                
            yield ContrapuntalPair(bass=bass, soprano=soprano, tonic=current_tonic, is_minor=is_minor)
...
def extract_outer_voices(notes_file_path: str | Path) -> Tuple[list[NoteInfo], list[NoteInfo]]:
    ...
    with open(notes_file_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            current_time = onset
            active_notes.append(ActiveNote(midi=midi, onset=onset, duration=duration, end=onset+duration, staff=staff))
...
def get_aria_outer_voice_pairs_fuzzy (file_name: str, max_sync_distance: int = 2) -> Iterator[ContrapuntalPair]:
    ...
    return generate_fuzzy_pairs(bass_notes, soprano_notes, max_sync_distance=max_sync_distance, harmony_file=harmony_file_path)
