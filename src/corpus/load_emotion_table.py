...
def load_emotion_table() -> list[EmotionLabelModel]:
    ...
    df_subset = df[list(emotion_table_model_map.keys())].rename(
        columns=emotion_table_model_map
    )
...
def get_arias_by_basic_passion() -> dict[str, list[AriaHeaderModel]]:
    ...
    return {
        emotion: [aria for label in labels for aria in aria_index.get(label, [])]
...
def create_or_get_basic_passion_by_aria_id_map():
    ...
    if not BASIC_PASSION_OF_ARIA_MAP_PATH.is_file():
        print(
            f"No aria index found. Generating new basic passion of aria map at {BASIC_PASSION_OF_ARIA_MAP_PATH}."
        )
...
def get_basic_passion_by_aria_id(
    aria_id: int, aria_emotion_grouping: dict[str, list[AriaHeaderModel]] | None = None
):
